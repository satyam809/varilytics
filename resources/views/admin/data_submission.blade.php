        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card-body table-responsive">
                    <table id="dataSubmissionTable" class="table table-hover">
                        <thead>
                            <th><input type="checkbox" id="select_all"></th>
                            <th>Name</th>
                            <th>Email</th>
                            <th>Dataset(or App)</th>
                            <th>Want</th>
...
<script>
    ...
    function contactFetch() {
        ...
        $.get('/admin/data_submission_data', function(data) {
            //console.log(data);
            ...
            $.each(JSON.parse(data), function(key, value) {
                $("#allContacts1").append(`<tr id="${value.id}">
                           ...
                           <td>${value.email}</td>
                           <td>${value.suggest}</td>
                           <td>${value.want_with_data}</td>
                           <td>${value.source_of_data}</td>
                           <td><a href="{{asset('assets/admin_assets/contact_file')}}/${value.file}" target="_blank" class="btn btn-default">View</a></td>

                        </tr>`);
                i++;
            });

            $("#dataSubmissionTable").dataTable();
        ...
        $('#delete_records').on('click', function(e) {
            ...
            if (contact.length <= 0) {
                ...
                if (checked == true) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/admin/data_submission_delete/" + selected_values,
                        dataType: "json",
                        cache: false,
                        data: {
                            "id": selected_values,
